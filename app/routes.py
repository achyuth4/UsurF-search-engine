...
def auth_required(f):
    ...
    def decorated(*args, **kwargs):
        ...
        UsurF_user = os.getenv('UsurF_USER', '')
        UsurF_pass = os.getenv('UsurF_PASS', '')
        if (not UsurF_user or not UsurF_pass) or (
                auth
                and UsurF_user == auth.username
                and UsurF_pass == auth.password):
            session['auth'] = True
            return f(*args, **kwargs)
        else:
...
def search():
    ...
    # removing st-card to only use UsurF time selector
...
def run_app() -> None:
    parser = argparse.ArgumentParser(
        description='UsurF Search console runner')
    parser.add_argument(
        '--port',
        default=5000,
        metavar='<port number>',
    ...
    if args.userpass:
        ...
        os.environ['UsurF_USER'] = user_pass[0]
        os.environ['UsurF_PASS'] = user_pass[1]

    if args.proxytype and args.proxyloc:
        if args.proxyauth:
            proxy_user_pass = args.proxyauth.split(':')
            os.environ['UsurF_PROXY_USER'] = proxy_user_pass[0]
            os.environ['UsurF_PROXY_PASS'] = proxy_user_pass[1]
        os.environ['UsurF_PROXY_TYPE'] = args.proxytype
        os.environ['UsurF_PROXY_LOC'] = args.proxyloc
    ...
    if args.debug:
        ...
        waitress.serve(
            ...
            url_prefix=os.environ.get('UsurF_URL_PREFIX', ''))
