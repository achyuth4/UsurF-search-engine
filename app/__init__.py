...
app.config['DEFAULT_CONFIG'] = os.path.join(
    ...
    'config.json')
app.config['CONFIG_DISABLE'] = read_config_bool('UsurF_CONFIG_DISABLE')
...
# Session values
app_key_path = os.path.join(app.config['CONFIG_PATH'], 'UsurF.key')
...
# link. Setting this value to 'strict' causes UsurF to revalidate a new
...
app.config['RELEASES_URL'] = 'https://github.com/' \
                             'benbusby/UsurF-search/releases'
...
# related searches.
translate_url = os.getenv('UsurF_ALT_TL', 'https://farside.link/lingva')
