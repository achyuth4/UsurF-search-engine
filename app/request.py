...
def gen_user_agent(is_mobile) -> str:
    user_agent = os.environ.get('UsurF_USER_AGENT', '')
    user_agent_mobile = os.environ.get('UsurF_USER_AGENT_MOBILE', '')
...
class Request:
    ...
    Attributes:
        ...
        root_path: the root path of the UsurF instance
        config: the user's current UsurF configuration
    """

    def __init__(self, normal_ua, root_path, config: Config):
        self.search_url = 'https://www.google.com/search?gbv=1&num=' + str(
            os.getenv('UsurF_RESULTS_PER_PAGE', 10)) + '&q='
        ...
        proxy_path = os.environ.get('UsurF_PROXY_LOC', '')
        if proxy_path:
            proxy_type = os.environ.get('UsurF_PROXY_TYPE', '')
            proxy_user = os.environ.get('UsurF_PROXY_USER', '')
            proxy_pass = os.environ.get('UsurF_PROXY_PASS', '')
            auth_str = ''
            if proxy_user:
    ...
    def send(self, base_url='', query='', attempt=0,
        ...
        """
        use_client_user_agent = int(os.environ.get('UsurF_USE_CLIENT_USER_AGENT', '0'))
