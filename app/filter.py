...
class Filter:
    ...
    def collapse_sections(self) -> None:
        ...
        Returns:
            None (The soup object is modified directly)
        """
        minimal_mode = read_config_bool('UsurF_MINIMAL')
    ...
    def update_css(self) -> None:
        """Updates URLs used in inline styles to be proxied by UANsurF      using the /element endpoint.
    ...
    def update_link(self, link: Tag) -> None:
        ...
        if any(url in link_netloc for url in unsupported_g_pages):
            ...
            if 'google.com/preferences?hl=' in link_netloc:
                ...
                # Since we want all language config done through UANsurFe
